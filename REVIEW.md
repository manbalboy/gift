...
- [ ] 백엔드 API 서버에 엄격한 CORS 정책(`manbalboy.com`, `localhost`, `127.0.0.1`)을 적용하고 테스트 검증하기.
- [ ] 프론트엔드 내 아티팩트 렌더링을 위한 `<SafeArtifactViewer />` 컴포넌트 모듈화 및 DOM 살균 로직(Sanitization) 적용하기.
- [ ] `<SafeArtifactViewer />` 및 신규 UI 기능에 대한 프론트엔드 단위 및 Playwright E2E 테스트 코드 작성하기.
- [ ] 휴먼 게이트(승인/거절) API 동작 시 동시 요청 처리를 위한 중복 방지 및 멱등성 보장 로직 추가하기.
- [ ] 서버 백엔드에 SSE Keep-Alive Ping 전송 주기 추가 및 클라이언트(포트 `3100`) 측 SSE 자동 재연결 및 이벤트 복구 로직 구현하기.
- [ ] 프론트엔드 Audit Log 날짜 필터링 시 클라이언트 타임존 오프셋을 파라미터나 헤더로 전달하도록 수정하기.
- [ ] 24시간 이상 대기 중인 워크플로우를 탐지하는 백엔드 스케줄러 구현 및 Time Mocking 기법을 활용한 단위 테스트 작성하기.
