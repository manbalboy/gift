...
test('모바일 긴 메시지 Toast는 탭으로 확장되고 리사이즈 이후에도 화면 경계를 유지한다', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'workflow_id 오류 시뮬레이션' }).click();
...
test('4개 이상 알림 발생 후 일괄 닫기로 큐를 비울 수 있다', async ({ page }) => {
  ...
  await page.goto('/');
  const trigger = page.getByRole('button', { name: 'workflow_id 오류 시뮬레이션' });
  for (let i = 0; i < 4; i += 1) {
    await trigger.click();
  }
...
test('브라우저 Hover/Focus 상호작용 중에는 자동 닫힘이 일시 정지되고 해제 시 재개된다', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'workflow_id 오류 시뮬레이션' }).focus();
