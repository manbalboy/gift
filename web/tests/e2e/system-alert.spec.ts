...
test('모바일(320px)에서 필터 칩과 액션 버튼이 함께 동작하고 레이아웃이 유지된다', async ({ page }) => {
  ...
  await expect(page.locator('.system-alert-item.system-alert-error')).toHaveCount(4);

  await page.getByRole('button', { name: 'Warning' }).click();
  await expect(page.locator('.system-alert-item')).toHaveCount(4);
  await expect(page.locator('.system-alert-item.system-alert-warning')).toHaveCount(4);

  await page.getByRole('button', { name: 'All' }).click();
  await expect(page.locator('.system-alert-item')).toHaveCount(7);
  const visibleLevelCounts = await page.evaluate(() => {
    const items = Array.from(document.querySelectorAll('.system-alert-item'));
    return {
      error: items.filter((item) => item.classList.contains('system-alert-error')).length,
      warning: items.filter((item) => item.classList.contains('system-alert-warning')).length,
    };
  });
  expect((visibleLevelCounts?.error ?? 0) >= 1).toBeTruthy();
  expect((visibleLevelCounts?.warning ?? 0) >= 1).toBeTruthy();
...
test('스크롤을 위로 올리면 auto-scroll이 멈추고 최하단 복귀 시 재개된다', async ({ page }) => {
  ...
  await expect
    .poll(async () => {
      const text = await page.locator('.system-alert-scroll-state').textContent();
      return text?.trim();
    })
    .toBe('PAUSED');
  ...
  expect(beforeLoadMore).not.toBeNull();
  ...
  expect(Math.abs((pausedState?.scrollTop ?? 0) - (beforeLoadMore?.scrollTop ?? 0))).toBeLessThan(2);
...
test('3100 포트에서 가변 높이 대량 로그 스크롤이 점핑 없이 유지된다', async ({ page }) => {
  await page.setViewportSize({ width: 1366, height: 900 });
      ...
      body: JSON.stringify([
        {
          id: 1,
          name: 'Stress Flow',
          description: 'dynamic height stress test',
          graph: { nodes: [{ id: 'idea', type: 'task', label: 'Idea' }], edges: [] },
        },
      ]),
    });
  ...
  await page.route('**/api/logs/system-alerts**', async (route) => {
    const payload = Array.from({ length: 12000 }).map((_, idx) => {
      const tail = ' :: '.repeat((idx % 9) + 1);
      return {
        id: `stress-${idx}`,
        created_at: `2026-03-05T01:${String(Math.floor(idx / 60) % 60).padStart(2, '0')}:${String(idx % 60).padStart(2, '0')}Z`,
        level: idx % 5 === 0 ? 'error' : idx % 3 === 0 ? 'warning' : 'info',
        code: `STRESS_${idx}`,
        message: `loop-stage-${idx % 4} | ${'segment '.repeat((idx % 14) + 2)}${tail} http://test.com/api?v=1.0.`,
        source: 'loop-engine',
        context: { risk_score: 50 + (idx % 45) },
        risk_score: 50 + (idx % 45),
      };
    });
  ...
  const performanceResult = await page.evaluate(async () => {
    ...
    for (let step = 1; step <= 28; step += 1) {
      const targetTop = (list.scrollHeight - list.clientHeight) * (step / 28);
      list.scrollTop = targetTop;
      list.dispatchEvent(new Event('scroll', { bubbles: true }));
      await new Promise((resolve) => window.setTimeout(resolve, 22));

      const currentTop = list.scrollTop;
      const jump = Math.abs(currentTop - previousTop);
      if (jump > maxJump) {
        maxJump = jump;
      }
    ...
    const rendered = document.querySelectorAll('.system-alert-item').length;
    const firstCode = document.querySelector('.system-alert-item strong')?.textContent ?? '';
  ...
  expect(performanceResult).not.toBeNull();
  expect((performanceResult?.elapsed ?? 9999) < 3600).toBeTruthy();
  expect((performanceResult?.rendered ?? 0) < 120).toBeTruthy();
  expect((performanceResult?.maxJump ?? 0) > 0).toBeTruthy();
  expect((performanceResult?.scrollTop ?? 0) > (performanceResult?.maxScrollTop ?? 0) * 0.88).toBeTruthy();
  expect((performanceResult?.firstCode ?? '').startsWith('STRESS_')).toBeTruthy();
});
